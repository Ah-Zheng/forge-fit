...
function recalculateDashboard() {
    ...
    document.getElementById('stat-volume').innerHTML = `${totalVolume.toLocaleString()} <span class="unit">kg</span>`;
    document.getElementById('stat-sets').innerHTML = `${completedSetsCount} <span class="unit">組</span>`;
    ...
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', () => {
            navigator.serviceWorker.register('sw.js')
                .then(reg => {
                    console.log('ForgeFit PWA Service Worker registered successfully, scope:', reg.scope);
                })
                .catch(err => {
