...
// 5. 運動時長 (直接映射自資料庫 session 的 duration 欄位)
const workoutDuration = computed(() => props.session.duration || 45)
  ...
  <div class="dashboard-grid" style="animation: fadeInUp 0.4s ease forwards;">
    ...
    <div class="stats-card-wrapper full-width">
      ...
      <div class="stat-card" :style="{ position: 'relative', zIndex: showVolumeTooltip ? 100 : 1 }">
        ...
        <div class="stat-data">
          ...
          <div class="stat-label" style="display: flex; align-items: center; gap: 0.25rem;">
            ...
            <button 
              ...
              title="點擊查看訓練量科學原理"
              style="background: transparent; border: none; padding: 0.2rem; cursor: pointer; color: var(--color-cyan); display: flex; align-items: center;"
            >
        ...
        <div v-if="showVolumeTooltip" class="glass-tooltip-popover">
          ...
          <div class="popover-body">
            ...
            <p class="desc">
              這在健身科學中被稱為<strong>「總負荷量」</strong>，是評估<strong>漸進性超負荷 (Progressive Overload)</strong> 最關鍵的指標。只要這個數值隨時間推移穩定上升，就代表你的肌肉在持續進步、變強！
            </p>
            <p class="motivation">
      ...
      <div class="stat-card">
        ...
        <div class="stat-data">
          <div class="stat-value">
            {{ workoutDuration }} <span class="unit">分鐘</span>
          </div>
    ...
    <div class="glass-card chart-card full-width">
      ...
      <div class="chart-legend-custom" id="chart-legend">
        ...
        >
          ...
          ></span>
          <span>
            {{ m.label }}: <strong>{{ m.vol.toLocaleString() }} kg</strong> ({{ m.pct }}%)
          </span>
        </div>
      </div>
    </div>
